...
class NetNormWeightsRelu(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(NetNormWeightsRelu, self).__init__()
        self.representations = dict()
        self.encoder = nn.utils.weight_norm(nn.Linear(input_size, hidden_size))
        self.relu = nn.ReLU()
        self.decoder = nn.utils.weight_norm(nn.Linear(hidden_size, output_size))
    ...
    def forward(self, x):
        ...
        self.representations['hidden'] = out.data.cpu().numpy().copy()
        out = self.relu(out)
        self.representations['hidden_relu'] = out.data.cpu().numpy().copy()
        out = self.decoder(out)
        self.representations['decode'] = out.data.cpu().numpy().copy()
        return out

class NetNormWeightsSigmoid(nn.Module):
            def __init__(self, input_size, hidden_size, output_size):
                super(NetNormWeightsSigmoid, self).__init__()
                self.representations = dict()
                self.encoder = nn.utils.weight_norm(nn.Linear(input_size, hidden_size))
            ...
            def forward(self, x):
                ...
                self.representations['hidden_sigmoid'] = out.data.cpu().numpy().copy()
                out = self.decoder(out)
                self.representations['decode'] = out.data.cpu().numpy().copy()
                return out
