...
class MLIRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.part == 'train':
            ...
            if  idx in self.train_users and sample.sum() != 0:
                # import pdb; pdb.set_trace()
                idx_to_drop = (sample[self.train_users].nonzero()[0])
                idx_to_drop = self.train_users[idx_to_drop]
                # print (sample[idx_to_drop])


                # print(sample[self.train_users].nonzero())
                # print(idx_to_drop.shape)

                idx_to_drop = idx_to_drop[:int(len(idx_to_drop)*self.p)]
                # print(idx_to_drop.shape)
                sample[idx_to_drop] = 0
        elif self.part == 'test':
