...
class IBCF():
    ...
    def predict(self, targets, input_data):
        ...
        for user in targets:
            ...
            disorder = np.argpartition(self.prediction_[user,:],self.item_num - self.rec_length)[-1 * self.rec_length:]
            # order the recomendataion list, descending order
            # import pdb; pdb.set_trace();

            # self.recommendation_[user] = np.argsort(-1 * self.prediction_[user,disorder])
            self.recommendation_[user] = disorder
