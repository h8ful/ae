...
class MLIRDataset(Dataset):
    def __init__(self, root_dir, cv, part, tranform = None):
            ...
            self.train_shrinked = self.train[self.train_users,:]
            self.test = mmread(os.path.join(root_dir, 'eval.%s' % cv)).A.astype(np.float32)
            self.targets = np.unique(self.test.nonzero()[0])
            self.newusers_shrinked = self.newusers[self.targets,:]
            self.part = part
            self.transform = tranform
    def __len__(self):
        return self.train_shrinked.shape[1]

    def __getitem__(self, idx):
        if self.part == 'train':
            sample = self.train_shrinked[:,idx]
        elif self.part == 'test':
            sample = self.test_shrinked[:,idx]
