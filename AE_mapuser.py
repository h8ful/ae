...
for epoch in range(num_epochs):
    for i_batch, sample_batched in enumerate(data6.train_loader):
        net.train()
        id_batch = sample_batched['id'].data.numpy()
        sample = Variable(sample_batched['sample']).cuda()
        target = Variable(torch.from_numpy(data6.test_dataset.newusers_shrinked[:,id_batch])).cuda()

        # Forward + Backward + Optimize
        optimizer.zero_grad()  # zero the gradient buffer
        outputs = net(sample)
        loss = certeria(outputs, target)
#         print(i_batch, loss.data[0])
    ...
    if (iters) % 1 == 0:
        ...
        input_matrix = Variable(torch.from_numpy(data6.train_dataset.train_shrinked.T)).cuda()
        mapped = net(input_matrix).cpu()
...
#         print('computing mse...')
#         test_user_mask = np.zeros_like(data5.train_dataset.known)
#         test_user_mask[data5.test_dataset.targets,:] = 1
#         test_user_mask = Variable(torch.from_numpy(test_user_mask))
#         train_mse[iters] = weighted_average(reconstructed,
#                                             Variable(torch.from_numpy(data5.train_dataset.known)),
#                                             (1-test_user_mask)).data.numpy()[0]
#         test_mse[iters] = weighted_average(reconstructed,
#                                            Variable(torch.from_numpy(data5.train_dataset.known)),
#                                            (test_user_mask)).data.numpy()[0]
#         all_mse[iters] = certeria(reconstructed,Variable(torch.from_numpy(data5.train_dataset.known.T))).data.numpy()[0]
# #         print((reconstructed.size()))

        print('prediction, computing precision...')
        mapped = reconstructed.data.numpy()
        mapped = reconstructed.T
#         print((reconstructed.shape))

        train_rec = rec_pred(reconstructed, train = np.zeros_like(data5.train_dataset.known), test = data5.train_dataset.known,
                             targets = data5.train_dataset.train_users)
        test_rec = rec_pred(reconstructed, train = data5.train_dataset.known, test = data5.test_dataset.test,
                             targets = data5.test_dataset.targets)
...
#             for sim in ['dot']:
            for sim in ['dot','asymcos','cosine']:
                ...
                cf.fit(train_ratings=data5.train_dataset.train, profile = representations[iters][key])

#                 for knn in [5]:
                for knn in [5,50,100,200,500]:
                        print ("CF (%s,%s,%s) on representations..."%((key,sim,knn)))

                        cf.compute_score(input_ratings = data5.train_dataset.newusers, topN = knn ,
                                                 targets=data5.test_dataset.targets)
                        cf.produce_reclist(targets=data5.test_dataset.targets)
                        tmp_cf_perf = cf.evaluate(test=data5.test_dataset.test, rec_len = 5)
